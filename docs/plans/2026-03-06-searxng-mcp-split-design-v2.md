...
const INTENT_ENGINE_MAP: Record<SearchIntent, { zh: string; en: string }> = {
  ...
  news:     {
    zh: 'google news,bing news,yahoo news,duckduckgo news,wikinews,startpage news,brave.news,reuters,qwant news',
...
const CODE_ACTION_WORDS = /报错|错误|安装|依赖|配置|部署|编译|调试|框架|代码|函数|接口|\b(error|bug|install|import|function|class|api|sdk|debug|deploy|config|compile|migrate|setup)\b/i;
...
├── searchMcp.ts                # [新增] web_search MCP
...
| 并发抓取过多 | 目标网站封 IP | 最大并发 5，Firecrawl 内置并发控制 (max 3) |
...
| 社区关键词 | `"React vs Vue 对比"` | code (tech+动作词优先) | Tier 2 |
