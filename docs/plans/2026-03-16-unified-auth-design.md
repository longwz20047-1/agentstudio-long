...
| `fetchActiveSessions` | `GET /api/agents/sessions` | menu.vue（传 `s.apiKey`） | ADMIN_PASSWORD 明文 |
| `testA2AConnection` | 内部调 `fetchA2AProjects` | ServerManager | ADMIN_PASSWORD 明文 |

> **`fetchActiveSessions` 说明**：该函数原有两个调用方。`ActiveSessionsPanel` 也调用此函数做初始加载，但 `ActiveSessionsPanel` 已通过 WebSocket `session:update` 事件实时接收 session 数据。REST 初始加载是冗余的——本次改造移除 `ActiveSessionsPanel` 中的 REST 调用，完全依赖 WebSocket 推送。移除后 `fetchActiveSessions` 仅 menu.vue 一个调用方，属于纯 A1 子类。

**子类 A2 — 传入 `agt_proj_*`（来自 `A2AConfig.apiKey`）**：
...
影响函数：`fetchA2AProjects`, `fetchA2AMapping`, `fetchA2AApiKeys`, `fetchActiveSessions`（`testA2AConnection` 内部调 `fetchA2AProjects`）
...
受影响函数：`fetchA2AProjects`, `fetchA2AMapping`, `fetchA2AApiKeys`, `fetchActiveSessions`（`testA2AConnection` 内部调 `fetchA2AProjects`，自动受益）

**A2 子类 — `getA2AHistory`**：当前接收 `A2AConfig`（含 `agt_proj_*`），函数内部没有 ADMIN_PASSWORD。

改造方案：**改函数签名**，不再接收 `A2AConfig`，改为显式传入 `serverUrl` + `adminPassword`：

```typescript
// 改造前
export async function getA2AHistory(config: A2AConfig, projectPath: string, sessionId: string) {
  const url = `${config.serverUrl}/api/a2a/history/${encodedPath}/${sessionId}`
  headers: { 'Authorization': `Bearer ${config.apiKey}` }  // ← agt_proj_*
...
export async function getA2AHistory(
  ...
  const jwt = await getToken(serverUrl, adminPassword)
  const url = `${serverUrl}/api/a2a/history/${encodedPath}/${sessionId}`
  headers: { 'Authorization': `Bearer ${jwt}` }             // ← AgentStudio JWT
```

调用方改造（`a2a-chat/index.vue:1829`，唯一调用方）：

```typescript
// 改造前
const history = await getA2AHistory(configStore.config, meta.project_path, meta.a2a_session_id)

// 改造后
const server = loadServers().find(s => s.serverUrl === meta.server_url)
const history = await getA2AHistory(
  meta.server_url,
  server?.apiKey || '',       // ← ADMIN_PASSWORD 从 serverStorage 取
  meta.project_path,
  meta.a2a_session_id
)
```

> 只改 2 个文件（`index.ts` 签名 + `index.vue` 调用方），不污染 `A2AConfig` 类型。

**`ActiveSessionsPanel` 改造（移除冗余 REST 调用）**：

`ActiveSessionsPanel` 原有两种数据来源：① REST `fetchActiveSessions` 初始加载 ② WebSocket `session:update` 实时推送。REST 调用是冗余的——WebSocket 连接后就会推送 session 数据。移除 REST 调用后，`ActiveSessionsPanel` 不再需要 `apiKey` prop：

```typescript
// 改造前
interface Props { serverUrl: string; apiKey: string; projectPath: string }
onMounted(() => { loadSessions() })  // REST 初始加载

// 改造后
interface Props { serverUrl: string; projectPath: string }  // 移除 apiKey
// 删除 loadSessions()、fetchActiveSessions import、watch apiKey
// 保留 WebSocket session:update handler（已有）
```

> `index.vue` 模板中 `<ActiveSessionsPanel>` 对应移除 `:api-key` prop。
...
| 2 | weknora-ui | `api/a2a/index.ts` | 改 | A1: 4 函数加 getToken + 401 重试；A2: getA2AHistory 改签名 (serverUrl, adminPassword, ...) |
...
| 8 | weknora-ui | `views/a2a-chat/components/ActiveSessionsPanel.vue` | 改 | 移除 REST 初始加载 + apiKey prop，完全依赖 WebSocket 推送 |
| 9 | weknora-ui | `views/a2a-chat/index.vue` | 改 | ActiveSessionsPanel 移除 :api-key；getA2AHistory 调用改为传 serverUrl + adminPassword |
| 10 | agentstudio | `middleware/auth.ts` | 改 | async IIFE + await verifyToken |
| 11 | agentstudio | `services/websocketService.ts` | 改 | authenticateToken 改用 verifyToken |
