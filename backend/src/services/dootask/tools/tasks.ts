...
收到"创建任务"请求时，LLM 的**第一个回复**必须按以下决策：

▶ 用户消息**已包含**项目名/项目 ID（如"测试产品开发项目加任务"）
  → 进入 Phase 1 信息收敛流程

▶ 用户消息**没有**项目名（如"创建任务"/"新建任务"/"加个任务"）
  → 第一动作 **必须** 是 list_projects() 拿项目候选
  → 第一句话 **必须** 是反问项目，列出候选让用户选：
     "您要在哪个项目下创建任务？以下是您参与的项目：
       1) 项目 A
       2) 项目 B
       3) 项目 C
       请选择（数字或名称）。"
  → ❌ 禁止先问"任务内容是什么"/"负责人是谁"/"优先级如何"
       → 必须先确定项目，再收集其他信息（项目是一切的容器）

错误示范（必须避免）：
  User: "创建任务"
  LLM: "请告诉我任务的具体内容和负责人，我来帮您创建。"  ← ❌ 跳过项目了
  LLM: "请问任务名称？"                                   ← ❌ 同样错
  LLM: "好的，我帮您创建一个任务，请提供详细信息。"        ← ❌ 没列出项目候选

正确示范：
  User: "创建任务"
  LLM: [list_projects] → "您要在哪个项目下创建？项目候选：
